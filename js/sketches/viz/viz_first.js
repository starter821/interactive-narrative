(function () {
  ...
  function interp(p) {
    ...
    function _basket(cd){
      ...
      for(let i=0;i<ITEMS.length;i++) _icon(ITEMS[i],POS[i][0],POS[i][1],22,cd[ITEMS[i]].qty);
    }

    function _icon(name,x,y,s,qty){
    ...
    p.mousePressed=function(){
      if(p.abs(p.mouseY-SL.y)<24&&p.mouseX>=SL.x1-30&&p.mouseX<=SL.x2+30){
        dragging=true;
        let ni=idxForX(p.constrain(p.mouseX,SL.x1,SL.x2));
